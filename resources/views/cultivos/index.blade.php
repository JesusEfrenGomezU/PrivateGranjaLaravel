    ...
    <div class="card">
        <div class="card-header">
            <div class="row">
                ...
                <div class="col-md-1">
                    <a href="{{ route('cultivos.create') }}" class="btn btn-primary"><i class="bi bi-plus-circle-fill"></i></a>
                </div>
            </div>
        </div>
        ...
        <div class="card-body">
            {{-- Paginador --}}
            <form action="{{ route('cultivos.index') }}" class="navbar-search" method="GET">
                <div class="row mt-3">
                ...
                <tbody>
                    @foreach ($cultivos as $cultivo)
                        <tr>
                            <td>{{ $cultivo->id }}</td>
                            <td>{{ $cultivo->tipo }}</td>
                            <td>{{ $cultivo->siembra }}</td>
                            <td>{{ $cultivo->cosecha }}</td>
                            <td>{{ $cultivo->estado }}</td>
                            <td>
                                {{-- Botón edit --}}
                                <a href="{{ route('cultivos.edit', $cultivo->id) }}" class="btn btn-warning">
                                    <i class="bi bi-pencil-fill"></i>
                                </a>
                                {{-- Botón delete --}}
                                <form action="{{ route('cultivos.delete', $cultivo->id) }}" style="display: contents;" method="POST">
                                    @csrf
                                    @method('DELETE')
            ...
            {{-- Paginador inferior --}}
            {{ $cultivos->appends(request()->except('page'))->links('components.customPagination') }}
        </div>
    </div>
</section>
@endsection
